...
test.describe('the generated site runs', () => {
  ...
  test('the first stretch of the flow walks through, page after page', async ({ page }) => {
    ...
    for (let step = 0; step < 10; step += 1) {
      ...
      // Nothing hidden all the way down. A page whose whole body is conditional renders as an empty
      // `<main>` with a Next button under it when the condition is false, and looks like a working
      // page to every assertion above.
      await expect(page.locator('#page-content-wrapper')).not.toBeEmpty()
  ...
  test('a screen with nothing to show is skipped rather than rendered empty', async ({ page }) => {
    // The breather after the About You questions: one conditional `<div>` holding two headings and no
    // question. Its condition is `isTrue(/aboutYouDataWasSaved)`, which Direct File's backend sets on a
    // save this port has none of, so the screen has nothing to render for anybody. Left in the
    // traversal it was a blank page between two real ones, and Next worked, so it read as a bug in the
    // page before it.
    //
    // The route is looked up rather than typed, so a flow that no longer has this screen fails saying so.
    const route = manifestRoutes.find((r) => r.endsWith('/about-you-breather'))
    expect(route, 'the flow should still have the About You breather').toBeDefined()
    ...
    expect(new URL(page.url()).pathname).not.toBe(`${APP}${route}/`)
    await expect(screenHeading(page)).toBeVisible()
    await expect(page.locator('#page-content-wrapper')).not.toBeEmpty()
    expect(errors).toEqual([])
  })
