...
function emit(extracted: Extracted, content: Content, resourcesDir: string) {
  ...
  // Read before the first gate is named, not just before they are written: `GateSet` reserves each
  // gate's name against these as well as against the gates already handed out. See reserveName.
  ...
  // The other direction of the same namespace check `reserveName` makes, asserted on the way out.
  // Cheap, and it fails with a message rather than the hang a duplicate `<Fact path>` produces: the
  // Fact Graph lets the second definition replace the first, so a gate that shadows the fact its own
  // body depends on becomes a cycle `FactDictionary.freeze` never leaves.
  const collisions = gates.filter((gate) => declared.has(gate.factPath)).map((g) => `${g.factPath} (from ${g.screens[0]})`);
  if (collisions.length > 0) {
    throw new Error(
      `${collisions.length} gate(s) are named after a fact the dictionary already declares:\n  ` +
        `${collisions.join(`\n  `)}`
    );
  }
