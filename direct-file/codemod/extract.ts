...
import { collectionItemName, resolveContent } from './resolve-content.ts';
...
function extractLoop(loop: FlowCollectionLoop) {
  return {
    ...
    // The English noun `<fg-collection item-name>` needs. Its Spanish twin comes out of the same
    // two sources in stage 15; see `collectionItemName` in resolve-content.ts.
    itemName: collectionItemName(en, `en`, loop.collectionName as string),
...
// Stage 4, over the default language. `translate.ts` runs the same function over `es.yaml`.
const content = resolveContent(en, `en`, extracted.screens, extracted.pages);
