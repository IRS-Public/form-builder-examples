...
| `Screen` (713) | one `<div class="df-screen" condition="/isYoungerThan16" operator="isTrue">`, in run order inside its page |
...
references it by name, e.g. `condition="/isYoungerThan16"`. Every operator expands to `All`, `Any`,
`Not` and `IsComplete` over `Dependency`, all four of which are in fact-graph's `defaultFactories`.

798 gates, 533 at the root and 265 on a collection item. 211 are shared by more than one screen. A
gate is named for the conditions it is built from rather than numbered: `gateName` in `gates.ts`
spells each condition as the claim it makes (`is…`, `isNot…`, `…IsKnown`) and joins them with `And`.
Naming rather than numbering means an unrelated change upstream does not renumber every gate below
it, and identical condition sets collapse to one fact whatever order they were written in.
...
A gate is defined wherever its paths live: at `/theCollection/*/isSomeCondition` when they carry
...
`path` when it clones a collection item, so the div's `condition="/formW2s/*/isImported"`
becomes `/formW2s/#id/isImported` for that item, the same substitution the questions get.
...
The same set of declared paths is read twice more. `GateSet` takes it as a constructor argument and
reserves each gate's name against it, which is the fix described under stage 3. And the emitter
asserts on the way out that no gate it is about to write is named after a declared fact, which is
redundant while the reservation holds and costs three lines to keep a regression from coming back as
...
`/familyAndHousehold/*/isFilersCouldHaveDependentOrQualifyingPersonOrIncompleteAndIsRelationshipCategoryIsChild`
appears inside `<fg-collection path="/cdccQualifyingPeople">`. That is correct rather than a leak.
