...
//               workspace-locked ("true"/"false"), context-label (breadcrumb
//               context override), menu-json (JSON string override)
...
// formative-studio's Fact Explorer, which *is* a Taxpert Workspace destination — rather
// than a per-host on/off preference to track and hand across a cross-origin nav.
//
...
class TaxpertGlobalNav extends HTMLElement {
  static get observedAttributes () {
    return ['active', 'workspace-on', 'workspace-locked', 'workspace-label', 'context-label', 'app', 'menu-json']
  }
  ...
  _renderBreadcrumb (active, menu) {
    ...
    // off the nav reverts to its starting state — just the "Taxpert" root. A host
    // can override the label via `context-label` for a location that isn't a menu
    // destination (e.g. formative-studio's landing page → "Taxpert Home"); otherwise
    // it's derived from the active menu item (e.g. /studio → "Fact Explorer").
    const ctx = this.workspaceOn ? (this.getAttribute('context-label') || contextLabel(active, menu)) : null
    if (ctx) {
