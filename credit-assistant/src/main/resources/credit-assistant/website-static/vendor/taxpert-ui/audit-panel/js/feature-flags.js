...
function _writeOverrides (overrides) {
  try {
    localStorage.setItem(FEATURE_FLAGS_KEY, JSON.stringify(Object.fromEntries(overrides)))
...
export function setFlag (name, value) {
  const overrides = _readOverrides()
  overrides.set(name, Boolean(value))
...
export function initFeatureFlagsSection () {
  ...
  if (hint) hint.textContent = `Build default: ${defaults.get('aiMode') ? 'on' : 'off'}`
