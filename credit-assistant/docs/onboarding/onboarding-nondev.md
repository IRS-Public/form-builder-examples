# Non-Dev EITC Assistant 2.0 Local Setup
**Table of Contents**
...
- [Run EITC Assistant Locally](#run-eitc-assistant-locally)
  - [Compile FactGraph](#compile-factgraph)
  - [Run EITC Assistant](#run-eitc-assistant)
  - [Kill EITC Assistant](#kill-eitc-assistant)
  - [Update EITC Assistant](#update-eitc-assistant)
...
2. Set your local `git` email address for authoring commits
    1. Configure your local `git` `user.email` setting
        * If you would like to use your no-reply email address for _all_ local development, run
           ...
           ```
        * If you prefer to only use the no-reply email address for development in your EITC Assistant working copy, run
          ```shell
          # Ensure you're in the directory for this git project
          cd ./path/to/eitc-assistant/
...
* In GitHub Desktop, clone the `fact-graph` & `eitc-assistant` repos to your ~/Documents/GitHub folder
...
* Once you've completed the above steps you're ready to [compile the fact graph](#compile-factgraph) and [Run EITC Assistant Locally](#run-EITC Assistant)
...
## Run EITC Assistant Locally
### Compile FactGraph
Before you can run EITC Assistant you'll need to compile the fact-graph.
* In terminal, move to the fact-graph repo & compile the fact-graph
    ...
    * Note: you'll need to do this every time the fact-graph repo updates, but not every time you run EITC Assistant

### Run EITC Assistant
Once you've compiled the fact-graph and run `make publish` you're ready to run EITC Assistant locally
* In terminal, move to the eitc-assistant repo & run it!
    * Open terminal and nav to your EITC Assistant repo: `cd ~/Documents/GitHub/eitc-assistant`
    * Run `make` (this will run and keep running)
* With EITC Assistant running in the background, you can open your browser and navigate to: http://localhost:3000/
* Note: you'll need to do this every time you want to run EITC Assistant (or just leave it running in the background)

### Kill EITC Assistant
* Use `ctrl+c` to kill the process
* From here you can quit terminal

### Update EITC Assistant
...
* This can be done while EITC Assistant is running in the background. It should update automatically
